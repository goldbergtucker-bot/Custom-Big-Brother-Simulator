...
/* =========================================================
   BIG BROTHER SIMULATOR — FINAL PRESENTATION / FINALE FIX
   Fixes:
   - left week/event tab being one step ahead
   - explicit Final HOH Parts 1, 2 and 3
   - Final 3 eviction before jury voting
   - stable dark eviction-vote presentation
   - complete 1st–16th placement memory wall
   ========================================================= */
(function installFinalPresentationFix(){
    function sim(){
        if(!currentSeason) return null;
        currentSeason.simulation=ensureFinaleState(currentSeason.simulation||createDefaultSimulation());
        return currentSeason.simulation;
    ...
    function player(id){return getHouseguestForSimulation(id);}
    function nm(id){return getHouseguestDisplayName(id,currentSeason?.houseguests||[]);}
    function active(){return getActiveHouseguests();}
    function finalists(){return (sim()?.finalists||[]).map(player).filter(Boolean);}

    function finalChain(){
        const f=finalists();
        const chain=[];
        if(f.length>=3){
            chain.push({key:'final-hoh-1',label:'Final HOH — Part 1'});
            chain.push({key:'final-hoh-2',label:'Final HOH — Part 2'});
            chain.push({key:'final-hoh-3',label:'Final HOH — Part 3'});
            chain.push({key:'final-eviction',label:'Final 3 Eviction'});
        }
        chain.push({key:'jury-voting',label:'Jury Voting'});
        chain.push({key:'finale-results',label:'Final Results'});
        return chain;
    }

    function finalPoolForPart(part){
        const f=finalists();
        if(part===1) return f;
        const s=sim();
        if(part===2){
            const winner=player(s.finalHOH1);
            return f.filter(p=>!winner||p.id!==winner.id);
        }
        const a=player(s.finalHOH1), b=player(s.finalHOH2);
        return f.filter(p=>p.id===a?.id||p.id===b?.id);
    }

    function runFinalPart(part){
        const s=sim(), pool=finalPoolForPart(part);
        if(pool.length<1){s.currentEventIndex++; return;}
        let comp=null;
        const list=currentSeason.competitions?.finalHoh || currentSeason.competitions?.finalHOH || [];
        if(Array.isArray(list)) comp=list.find(c=>Number(c.part||c.finalPart||0)===part)||list[part-1]||null;
        const primary=part===1?'physical':part===2?'mental':'mental';
        const secondary=part===1?'endurance':part===2?'physical':'general';
        const winner=chooseWinnerForFinalFix(pool,comp,primary,secondary);
        ...
        const description=part===1
            ? 'All three Finalists compete in the endurance-based Part 1.'
            : part===2
                ? 'The two Houseguests who did not win Part 1 compete in Part 2.'
                : 'The Part 1 and Part 2 winners face off in the Final HOH Part 3.';
        const content=`<div class="final-hoh-event"><div class="final-hoh-part-badge">PART ${part}</div>${simulationPortrait(winner,'large')}<h3>${escapeHTML(nm(winner.id))}</h3><p><strong>${escapeHTML(comp?.name||('Final HOH — Part '+part))}</strong></p><p>${description}</p></div>`;
        showFinalFixEvent(s,comp?.name||('Final HOH — Part '+part),'FINAL HOH',content,'final-hoh-'+part);
    }

    function chooseWinnerForFinalFix(players,comp,primary,secondary){
        if(comp) return chooseCompetitionWinnerByCustom(players,comp);
        return chooseCompetitionWinner(players,primary,secondary,'general');
    }

    function runFinalEvictionFix(){
        const s=sim(), f=finalists();
        if(f.length<=2){s.currentEventIndex++;return;}
        const hoh=player(s.finalHOH3)||player(s.finalHOH1)||f[0];
        const choices=f.filter(p=>p.id!==hoh.id);
        const chosen=choices.slice().sort((a,b)=>allianceBond(hoh.id,b.id)-allianceBond(hoh.id,a.id))[0]||choices[0];
        chosen.status='evicted'; chosen.placement=3;
        s.finalists=f.filter(p=>p.id!==chosen.id).map(p=>p.id);
        const content=`${simulationPortrait(hoh,'large')}<p><strong>${escapeHTML(nm(hoh.id))}</strong> has won the Final HOH.</p><div class="final-eviction-choice"><h3>Final 3 Eviction</h3>${simulationPortrait(chosen,'large')}<p><strong>${escapeHTML(nm(chosen.id))}</strong> has been evicted and finishes in <strong>3rd Place</strong>.</p></div>`;
        showFinalFixEvent(s,'Final 3 Eviction','FINAL EVICTION',content,'final-eviction');
    }

    function showFinalFixEvent(s,title,type,content,event){
        showEvent(title,type,content,{skipHistory:true,skipLiveView:false,week:getSeasonLength(currentSeason)});
        if(!Array.isArray(s.history)) s.history=[];
        s.history.push({week:getSeasonLength(currentSeason),cycle:0,event,label:event,title,type,content,sequence:s.history.length,timestamp:Date.now()});
        s.weekHistory=s.history;
        s.liveView={title,type,content,week:getSeasonLength(currentSeason),event};
        s.currentEventIndex=Number(s.currentEventIndex||0)+1;
        updateSimulatorStatus(s); renderSimulationWeekNavigation(); renderMemoryWallMini(); persistCurrentSeason();
    }

    function runFixedNextEvent(){
        if(!currentSeason){alert('Please open a saved season first.');return;}
        const s=sim();
        if(s.isViewingHistory){returnToCurrentSimulation();return;}
        if(s.completed){showResults();return;}
        if(s.pendingWeekAdvance){
            ...
            if(next>getSeasonLength(currentSeason)){beginCleanFinale();return;}
            resetCycle(s,next,1); setText('current-week',next); resetGameChain(0);
            showEvent('Week '+next,'WEEK',`<p>Week ${next} is now beginning.</p>`,{skipHistory:true,week:next});
            persistCurrentSeason(); renderSimulationWeekNavigation(); return;
        }
        if(s.pendingCycle==='double'){s.pendingCycle=null;s.currentEventIndex=0;s.currentPhase='week';}
        if(s.currentPhase==='finale'||s.finaleStarted){
            const chain=finalChain(), index=Number(s.currentEventIndex||0), event=chain[index];
            if(!event){runCleanFinalResults();return;}
            if(event.key==='final-hoh-1')runFinalPart(1);
            else if(event.key==='final-hoh-2')runFinalPart(2);
            else if(event.key==='final-hoh-3')runFinalPart(3);
            else if(event.key==='final-eviction')runFinalEvictionFix();
            else if(event.key==='jury-voting')runCleanJuryVoting();
            else runCleanFinalResults();
            return;
        }
        const chain=cycleChain(s), index=Number(s.currentEventIndex||0), event=chain[index];
        if(!event){finishCycleAfterEviction(false);return;}
        switch(event.key){
            case 'twist':runCleanTwist(event.twistId);break;
            case 'hoh':runCleanHOH();break;
    ...
    /* The current event is the last completed history item; pending events follow it.
       This prevents the sidebar from appearing one event ahead of the center panel. */
    window.renderSimulationWeekNavigation=function(){
        const container=document.getElementById('sim-week-navigation'); if(!container||!currentSeason)return;
        const s=sim(), maxWeeks=getSeasonLength(currentSeason), currentWeek=Number(s.currentWeek||1), viewingWeek=Number(s.viewingWeek||currentWeek);
        const html=[];
        for(let w=1;w<=maxWeeks;w++){
            const current=w===currentWeek, viewing=w===viewingWeek, items=(s.history||[]).filter(h=>Number(h.week)===w);
            html.push(`<div class="sim-week-block ${current?'current':''} ${viewing?'viewing':''} ${w<currentWeek?'past':''}`);
            html.push(`<button type="button" class="sim-week-label" onclick="viewSimulationWeek(${w})">Week ${w}</button>`);
            if(viewing){
                html.push('<div class="sim-event-list">');
                items.forEach((item,i)=>html.push(`<button type="button" class="sim-event-nav completed" onclick="viewSimulationHistoryEvent(${w}, ${Number(item.sequence)})"><span>${escapeHTML(item.label||item.title||'Event')}</span></button>`));
                if(current&&!s.isViewingHistory&&!s.pendingWeekAdvance&&!s.pendingCycle){
                    const chain=s.currentPhase==='finale'||s.finaleStarted?finalChain():cycleChain(s);
                    const active=Number(s.currentEventIndex||0);
                    const pending=chain.slice(active);
                    if(pending.length) pending.forEach((item,i)=>html.push(`<div class="sim-event-nav ${i===0?'active':'pending'}"><span>${escapeHTML(item.label)}</span></div>`));
                }
                html.push('</div>');
            }
            html.push('</div>');
    ...
    window.runNextEvent=runFixedNextEvent;
    window.runFinalHOHEvent=()=>runFinalPart(1);

    /* Replace the results placement list with a memory-wall presentation. */
    window.renderFinalPlacements=function(){
        const c=document.getElementById('final-placements'); if(!c||!currentSeason)return;
        const s=sim();
        let placements=(s.finalPlacements||[]).slice();
        if(placements.length<currentSeason.houseguests.length){
            placements=currentSeason.houseguests.map(p=>({id:p.id,name:nm(p.id),placement:Number(p.placement||0)})).filter(x=>x.placement>0).sort((a,b)=>a.placement-b.placement);
        }
        placements.sort((a,b)=>a.placement-b.placement);
        c.innerHTML=`<div class="results-memory-wall">${placements.map(x=>{const p=player(x.id);const place=x.placement;const suffix=place===1?'st':place===2?'nd':place===3?'rd':'th';return `<div class="results-memory-card placement-${place}">${p?simulationPortrait(p,'medium'):''}<div class="results-memory-name">${escapeHTML(x.name||nm(x.id))}</div><div class="results-memory-placement">${place}${suffix} Place</div></div>`;}).join('')}</div>`;
    };

    /* Keep the simulator page dark during vote-reveal events. */
    const oldShowEvent=window.showEvent;
    window.showEvent=oldShowEvent;
})();
